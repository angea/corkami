; a PE with several sections with the same physical space, and the header too
...
istruc IMAGE_SECTION_HEADER
    at IMAGE_SECTION_HEADER.VirtualSize,      dd Section0Size
    at IMAGE_SECTION_HEADER.VirtualAddress,   dd VDELTA + Section0Start - IMAGEBASE + 1000h
    at IMAGE_SECTION_HEADER.SizeOfRawData,    dd iround(Section0Size, FILEALIGN)
    at IMAGE_SECTION_HEADER.PointerToRawData, dd Section0Start - IMAGEBASE
    at IMAGE_SECTION_HEADER.Characteristics,  dd IMAGE_SCN_MEM_EXECUTE | IMAGE_SCN_MEM_WRITE
iend
...
EntryPoint:
    call $ + 5
    pop eax
    cmp eax, VDELTA + EntryPoint + 5
    jz EntryPoint + SECTIONALIGN    ; jumping to section 2
    
    push VDELTA + Msg + SECTIONALIGN
    call [VDELTA + __imp__printf]
    add esp, 1 * 4
    
    db 0e9h                         ; jumping to header
        dd 200h - 2000h
_
    push 0
