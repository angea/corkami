...
\midrule
IsDbgPresent		& {\tt D.....} & $ PEB.BeingDebugged $ db {\tt [fs:[30] + 2]} == 1\\
IsDbgPresent2		& {\tt D....A} & $BeingDebugged$ check, via $IsDebuggerPresent$ \\
...
NtQueryInfo			& {\tt D....A} & {\small $NtQueryInformationProcess$}$(-1, ProcessDebugPort = 7, var, .....)$ \ra $[var] = present ? -1:0$ \\
HideThread			& {\tt D....A} & {\small $NtSetInformationThread$}$(-2, ThreadhideFromDebugger = 11, -1, 0)$ \ra not responding \\
...
\end{itemize}
\footnotetext{$70 = FLG\_HEAP\_ENABLE\_TAIL\_CHECK or FLG\_HEAP\_ENABLE\_FREE\_CHECK or FLG\_HEAP\_VALIDATE\_PARAMETERS$}
\footnotetext{$dword [7FFE0000] * dword[7FFE0004]) >> 24$}
\sig
\end{document}
